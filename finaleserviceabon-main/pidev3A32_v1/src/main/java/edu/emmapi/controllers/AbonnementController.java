...
public class AbonnementController {
    ...
    public void initialize() {
        ...
        abonnementTable.setPlaceholder(new Label("Aucun abonnement trouvé."));
    ...
    private void setupTableColumns() {
        serviceIdColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getService() != null ? cellData.getValue().getService().getNom() : "N/A"));
        typeAbonnementIdColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getTypeAbonnement() != null ? cellData.getValue().getTypeAbonnement().getNom() : "N/A"));
        dateDebutColumn.setCellValueFactory(new PropertyValueFactory<>("dateDebut"));
        dateFinColumn.setCellValueFactory(new PropertyValueFactory<>("dateFin"));
        estActifColumn.setCellValueFactory(cellData -> new SimpleBooleanProperty(cellData.getValue().isEstActif()).asObject());
        prixTotalColumn.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getPrixTotal()).asObject());
        estGratuitColumn.setCellValueFactory(cellData -> new SimpleBooleanProperty(cellData.getValue().isEstGratuit()).asObject());
        statutColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getStatut() != null ? cellData.getValue().getStatut().toString() : "N/A"));
        nombreSeancesRestantesColumn.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getNombreSeancesRestantes()).asObject());
        autoRenouvellementColumn.setCellValueFactory(cellData -> new SimpleBooleanProperty(cellData.getValue().isAutoRenouvellement()).asObject());
        dureeMoisColumn.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getDureeMois() != null ? cellData.getValue().getDureeMois() : 0).asObject());
        modePaiementColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getModePaiement() != null ? cellData.getValue().getModePaiement() : "N/A"));
    ...
    private void loadServices() {
        try {
            ...
            services.clear();
            services.addAll(serviceList);
            System.out.println("Loaded " + serviceList.size() + " service(s).");
            if (serviceList.isEmpty()) {
                showInfo("Information", "Aucun service trouvé dans la base de données.");
            }
        } catch (Exception e) {
            showError("Erreur lors du chargement des services", "Erreur: " + e.getMessage());
    ...
    private void loadTypeAbonnements() {
        try {
            ...
            typeAbonnements.clear();
            typeAbonnements.addAll(typeList);
            System.out.println("Loaded " + typeList.size() + " type(s) d'abonnement.");
            if (typeList.isEmpty()) {
                showInfo("Information", "Aucun type d'abonnement trouvé dans la base de données.");
            }
        } catch (Exception e) {
            showError("Erreur lors du chargement des types d'abonnements", "Erreur: " + e.getMessage());
    ...
    private void loadAbonnements() {
        try {
            ...
            abonnements.clear();
            abonnements.addAll(loadedAbonnements);
            System.out.println("Loaded " + loadedAbonnements.size() + " abonnement(s). Sort: " + sort);
            if (loadedAbonnements.isEmpty()) {
                showInfo("Information", "Aucun abonnement trouvé dans la base de données.");
            }
        } catch (Exception e) {
            showError("Erreur lors du chargement des abonnements", "Erreur: " + e.getMessage());
    ...
    private void handleSort() {
        ...
        if (selectedSort != null) {
            try {
                ...
                abonnements.clear();
                abonnements.addAll(sortedAbonnements);
                System.out.println("Sorted " + sortedAbonnements.size() + " abonnement(s) by " + selectedSort);
                showSuccess("Succès", "Abonnements triés par " + selectedSort);
            } catch (Exception e) {
                showError("Erreur lors du tri des abonnements", "Erreur: " + e.getMessage());
                System.err.println("Erreur lors du tri des abonnements: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
    ...
    private void handleAddAbonnement() {
        ...
        if (newAbonnement != null) {
            try {
                ...
                showError("Erreur lors de l'ajout de l'abonnement", "Erreur: " + e.getMessage());
        ...
        if (selectedAbonnement != null) {
            ...
            if (updatedAbonnement != null) {
                try {
                    ...
                    showError("Erreur lors de la mise à jour de l'abonnement", "Erreur: " + e.getMessage());
            ...
            confirm.showAndWait().ifPresent(response -> {
                if (response == ButtonType.YES) {
                    try {
                        ...
                        showError("Erreur lors de la suppression de l'abonnement", "Erreur: " + e.getMessage());
    ...
    private void handleReservation(Abonnement selectedAbonnement) {
        if (selectedAbonnement != null) {
            try {
                ...
                showError("Erreur lors de la réservation", "Erreur: " + e.getMessage());
