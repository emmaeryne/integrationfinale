...
public class produitpage1 {
    ...
    private void chargerProduits() {
        ...
        colCategorie.setCellValueFactory(cellData -> {
            ...
            int catId = cellData.getValue().getCategorie();
            // 2) récupérer le nom correspondant
            String nomCat = categorieService.getNomCategorieById(catId);
            // 3) retourner une SimpleStringProperty pour la colonne
            return new SimpleStringProperty(nomCat != null ? nomCat : "");
    ...
    private void filterProducts() {
        ...
        filteredList.setPredicate(p -> {
            // Affiche tout si la recherche est vide
            if (searchText.isBlank()) {
                return true;
            }

            // Récupère le nom de la catégorie à partir de l'ID
            String nomCat = categorieService.getNomCategorieById(p.getCategorie());
            if (nomCat == null) {
                nomCat = "";
            } else {
                nomCat = nomCat.toLowerCase();
            }

            // Vérifie les correspondances sur chaque champ
            return p.getNom_Produit().toLowerCase().contains(searchText)
                    || nomCat.contains(searchText)
                    || String.valueOf(p.getPrix()).contains(searchText)
                    || p.getDate().contains(searchText)
                    || p.getFournisseur().toLowerCase().contains(searchText);
        });
    }
        ...
        if (file != null) {
            try {
                ...
                for (produit p : produits) {
                    ...
                    // Récupère le nom de la catégorie depuis l'ID
                    String nomCat = categorieService.getNomCategorieById(p.getCategorie());
// Définit "Inconnu" si jamais c'est null
                    nomCat = nomCat != null ? nomCat : "Inconnu";
// Ajoute la cellule avec le libellé
                    table.addCell(new PdfPCell(new Phrase(nomCat, dataFont)));
