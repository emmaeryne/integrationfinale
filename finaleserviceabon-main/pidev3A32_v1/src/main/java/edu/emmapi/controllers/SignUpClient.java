...
public class SignUpClient {
    ...
    private void loadSecurityQuestions() {
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/hive3", "root", "")) {
    ...
    void handleSignUp(ActionEvent event) throws SQLException, IOException {
        ...
        if (EmailErrorLabel.getText().equals("Email valide ✓") &&
            ...
            if (motDePasse.equals(MotDePasse2Client.getText())) {
                String mdpHache = hashPassword(motDePasse); // Hash password with bcrypt (cost 13)
                System.out.println("Hashed Password: " + mdpHache);

                user newUser = new user(userName, mdpHache, email, active, role, 1, securityAnswer);
